...
export function IndianScoutExpansionHero({
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 bg-[#0a0a0a]/92 backdrop-blur-2xl text-white overflow-y-auto overflow-x-hidden selection:bg-[#ff2c2c] selection:text-black"
    >
      {/* Top Floating Header with Controls */}
      <div className="sticky top-0 z-50 w-full px-6 py-4 flex items-center justify-between bg-black/60 backdrop-blur-md border-b border-white/10">
        <div className="flex items-center gap-3">
          <button
            onClick={handleClose}
            className="bg-[#161616] hover:bg-[#ff2c2c] hover:text-black text-white px-4 py-2 rounded-lg border border-white/15 hover:border-[#ff2c2c] text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer active:scale-95"
          >
            <X className="w-4 h-4" />
            <span>Back to Main Screen</span>
          </button>
          <span className="text-white/40 hidden sm:inline-block">•</span>
          <span className="text-[#ff2c2c] font-black text-xs uppercase tracking-widest hidden sm:inline-block">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col items-center">
        ...
        <motion.div
          ...
          className="relative w-full max-w-5xl rounded-2xl overflow-hidden border border-white/20 shadow-[0_25px_90px_rgba(0,0,0,0.95)] aspect-[16/9] bg-[#111] mb-8"
        >
          <video
            ref={videoRef}
          ...
          <div className="absolute top-6 left-6 right-6 flex items-start justify-between z-20 pointer-events-none">
            <div>
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-[#ff2c2c] bg-black/60 px-2.5 py-1 rounded backdrop-blur-sm border border-[#ff2c2c]/30">
                {currentModelInfo.colorTag}
              </span>
              <h1 className="text-3xl sm:text-5xl font-black uppercase italic font-display text-white mt-2 tracking-tight drop-shadow-2xl">
                {currentModelInfo.name}
              </h1>
            </div>

            <div className="text-right">
              <span className="text-xs font-mono font-bold text-white/70 block">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between z-20">
            <div className="flex items-center gap-3">
              <button
                onClick={toggleMute}
                className="p-2 rounded-full bg-black/70 border border-white/20 text-white hover:text-[#ff2c2c] transition cursor-pointer backdrop-blur-sm active:scale-95"
              >
                {isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
              </button>
              <span className="text-[10px] font-mono text-white/80 uppercase bg-black/70 px-3 py-1 rounded border border-white/10 backdrop-blur-sm">
                SPEEDPLUS 1250 V-TWIN • 130 MPH
              </span>
            </div>

            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-2 bg-black/70 border border-white/15 px-3 py-1 rounded backdrop-blur-sm">
                <Gauge className="w-3.5 h-3.5 text-[#ff2c2c]" />
                <span className="text-xs font-mono font-bold text-[#ff2c2c]">{telemetrySpeed} MPH</span>
              </div>
              <button
                onClick={toggleEngineRev}
                className={`px-3 py-1 rounded text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer ${
                  isEngineRevving
                    ? 'bg-[#ff2c2c] text-black shadow-[0_0_20px_rgba(255,44,44,0.5)]'
                    : 'bg-black/70 text-white border border-white/20 hover:border-[#ff2c2c]'
                }`}
              >
        ...
        <motion.div
          ...
          className="w-full max-w-5xl bg-[#121212] border border-white/10 rounded-2xl p-6 sm:p-10 shadow-2xl space-y-8"
        >
          {/* Header & Overview */}
          <div className="border-b border-white/10 pb-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-3">
              <div>
                ...
                <h2 className="text-3xl sm:text-4xl font-black uppercase italic font-display text-white mt-1">
                  About The {currentModel.name}
                </h2>
              </div>
              <span className="bg-[#181818] border border-white/10 text-white text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-lg">
            ...
            <p className="text-sm sm:text-base text-white/60 leading-relaxed mt-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 bg-[#161616] p-4 rounded-xl border border-white/10">
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Horsepower</span>
              <span className="text-sm font-black font-mono text-[#ff2c2c]">{currentModelInfo.hp}</span>
            </div>
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Peak Torque</span>
              <span className="text-sm font-bold font-mono text-white">{currentModelInfo.torque}</span>
            </div>
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Displacement</span>
              <span className="text-sm font-bold font-mono text-white">{currentModelInfo.displacement}</span>
            </div>
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Seat Height</span>
              <span className="text-sm font-bold font-mono text-white">{currentModelInfo.seatHeight}</span>
            </div>
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Dry Weight</span>
              <span className="text-sm font-bold font-mono text-white">{currentModelInfo.dryWeight}</span>
            </div>
            <div>
              <span className="text-[9px] font-bold uppercase tracking-wider text-white/40 block">Fuel Tank</span>
              <span className="text-sm font-bold font-mono text-white">{currentModelInfo.fuelCapacity}</span>
            </div>
          </div>

          {/* Actions Bar */}
          <div className="flex flex-wrap sm:flex-nowrap gap-4 items-center justify-between border-t border-white/10 pt-6">
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="border border-white/20 hover:border-[#ff2c2c] hover:text-[#ff2c2c] bg-[#181818] text-white px-6 py-3.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 cursor-pointer active:scale-95"
              >
            ...
            <button
              ...
              className="text-white/60 hover:text-white text-xs font-bold uppercase tracking-wider underline cursor-pointer"
            >
              Explore Full Lineup →
            </button>
          </div>
