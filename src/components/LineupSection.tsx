...
export function LineupSection({ selectedModelId, onOpenCustomizer, onOpenTestRide }: LineupSectionProps) {
  ...
  return (
    <section id="lineup" className="relative w-full bg-[#0a0a0a] text-[#f5f5f5] pt-16 pb-12 overflow-hidden border-t border-white/5">
        ...
        <div className="w-full overflow-hidden relative">
          ...
          >
            {extendedLineup.map((bike, idx) => {
              ...
              const bikeImage = bike.colors[0]?.imageUrl || 'images/Indian-Scout.webp';
      ...
      {/* Model Information & Action Bar */}
      <div className="max-w-7xl mx-auto px-6 lg:px-10 mt-4 pt-6 border-t border-white/5">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          {/* Column 1: Model Title & Short Bio (7 cols) */}
          <div className="lg:col-span-7 space-y-1.5">
            <span className="text-[10px] text-[#ff2c2c] font-bold uppercase tracking-widest block">
            ...
            <p className="text-xs text-white/50 pt-1 leading-relaxed max-w-2xl">
              {currentModel.description}
            </p>
          </div>

          {/* Column 2: CTAs (Build & Price, Schedule Test Ride) (5 cols) */}
          <div className="lg:col-span-5 flex flex-wrap sm:flex-nowrap items-center justify-start lg:justify-end gap-3">
            <button
              onClick={() => onOpenCustomizer(currentModel)}
              className="w-full sm:w-auto bg-[#ff2c2c] hover:bg-white text-black px-6 py-3.5 rounded-sm text-xs font-black uppercase tracking-wider shadow-xl transition-all flex items-center justify-center cursor-pointer active:scale-95"
            >
              <span>Build & Price</span>
            </button>
            <button
              onClick={() => onOpenTestRide(currentModel)}
              className="w-full sm:w-auto border border-white/20 hover:border-[#ff2c2c] hover:text-[#ff2c2c] bg-[#161616] text-white px-6 py-3.5 rounded-sm text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center space-x-1.5 cursor-pointer active:scale-95"
            >
